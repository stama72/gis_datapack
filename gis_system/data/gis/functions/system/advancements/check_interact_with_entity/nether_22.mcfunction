execute as @a at @s if predicate gis:in_boss_n run execute in minecraft:gis_nether run tellraw @s {"text":"帰還します…"}
execute as @a at @s if predicate gis:in_boss_n run execute in minecraft:gis_nether run tp @s 234.00 49.00 -2.0 270 0
