...
execute as @s[advancements={gis:gameplay/growtree_ability_special/_11=true}] unless score @s gis_instant1 matches 1.. store success score @s gis_instant1 run tellraw @s {"text":"この要素はすでに解放されています"}
...
execute as @s unless score @s gis_instant1 matches 1.. if score @s gis_tp matches 32.. run tellraw @s [{"text":"[狂戦士]","color":"green"},{"text":"を開放しました","color":"white","bold":false}]
execute as @s unless score @s gis_instant1 matches 1.. if score @s gis_tp matches 32.. run advancement grant @s only gis:gameplay/growtree_ability_special/_11
