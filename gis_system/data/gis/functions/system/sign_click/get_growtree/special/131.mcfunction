execute as @s[advancements={gis:ability_special/13=false}] store success score @s instant run tellraw @s {"text":"グロウツリーの前提要素が解放されていません"}
...
execute as @s[advancements={gis:ability_special/131=true}] unless score @s instant matches 1.. store success score @s instant run tellraw @s {"text":"この要素はすでに解放されています"}
...
execute as @s unless score @s instant matches 1.. if score @s nowTP matches 64.. run tellraw @s [{"text":"[疾風迅雷]","color":"green"},{"text":"を開放しました","color":"white","bold":false}]
execute as @s unless score @s instant matches 1.. if score @s nowTP matches 64.. run advancement grant @s only gis:ability_special/131
